...
主代理默认直接实现资金、权限、事务、状态机与恢复等核心路径。Worker 只承接接口和预期已冻结、能够独立验收的外围工作；详细开发包和独立目录本身不足以证明适合委派。

- **按任务分工**：主代理处理核心判断与实现，Worker 承接可独立验收的外围任务。
...
| 执行者 | 主要职责 |
|---|---|
| Codex 主代理 | 理解需求、实现核心路径、整合与验收 |
| Luna Max | 原生子代理，处理预期已冻结的外围任务 |
