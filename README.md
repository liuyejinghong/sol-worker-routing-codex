...
| **DeepSeek** | 来源固定、输入很大、结果可机械验收 | 阅读固定网页或论文、查找代码事实、整理日志与结构化数据 |
...
DeepSeek 最有价值的地方，不只是“便宜地查一行代码”，而是把**大量固定材料压缩成少量、可核对的证据**：

| 输入材料 | DeepSeek 交付给 Sol 的结果 |
|---|---|
| 固定网页、论文、长文档 | 来源、日期、核心事实、支持与限制组成的证据表 |
| 大型代码库或大 diff | 调用位置、配置引用、重复模式和机械合同检查 |
...
并发也不再固定卡在两个。Sol 先派两个 Worker 验证任务合同；首批结果合格、剩余材料确实相互独立且只读时，可以自动把 DeepSeek 扩到**最多四个**。Luna Max 同时最多两个，且必须写入范围完全分离；只要涉及同一写入面，默认仍只开一个并按顺序执行。
