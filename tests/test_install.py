...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cwr-regression-") as folder:
        ...
        for previous in ("d058287492a01419922b17100f041abf1ac2d1eb", "v0.16.2", "f6ddb2b"):
            for disabled in (False, True):
                home = root / f"{previous}-{'disabled' if disabled else 'enabled'}"
                for source, target in (
                    ...
                    file.write_bytes((ROOT / source).read_bytes() if previous == "v0.16.2" and source.startswith("skills/")
                                     else subprocess.check_output(["git", "show", f"{previous}:{source}"], cwd=ROOT))
                result = run(home)
