...
Treat user-facing deliverables as real product content. UI, PDFs, presentations, reports, screenshots, and exports intended for end users, customers, or business audiences should contain only information those audiences need to complete the product or business task. Do not put Agent reasoning, implementation rationale, design deliberation, debugging history, or next-step plans into those deliverables. UI copy must not use development-report phrasing such as “I will,” “we can,” “this page demonstrates,” “this will,” “used to explain,” or “implemented,” unless it is genuinely necessary help, guidance, or empty-state copy. Keep implementation details, verification methods, engineering limitations, and technical tradeoffs in chat responses, code comments, PR/MR descriptions, engineering documentation, or planning files. A report may still present methods, assumptions, data limitations, or risks when they are necessary to understand or trust its conclusions, but never as a narration of the Agent's work. Include design notes, methodology, implementation process, or work logs in a final deliverable only when the user explicitly asks for them.

“Sol” below names the coordinator role; it does not require selecting `gpt-5.6-sol`. When the current parent is `gpt-5.6-terra` (or another supported parent model), it retains the same objective, routing, acceptance, authorization, and final-answer duties and may route eligible packets through the two Luna lanes.
...
把面向最终用户、客户或业务受众的交付物当作真实产品内容。UI、PDF、PPT、报告、截图和导出文件只呈现受众完成产品或业务任务所需的信息，不得写入 Agent 的思考过程、实现理由、设计推理、调试记录或下一步计划。UI 中不得出现「我将」「我们可以」「本页面用于展示」「这里会」「用于说明」「实现了」等开发汇报式或解释实现式文案，除非它本身是必要的帮助文本、操作指引或空状态文案。实现思路、验证方式、工程限制和技术取舍应放在聊天回复、代码注释、PR/MR 描述、工程文档或计划文件中。如果方法、假设、数据限制或风险说明是理解或信任报告结论所必需的，可以面向受众正常呈现，但不得写成 Agent 工作过程。只有用户明确要求展示设计说明、方法论、实现过程或工作记录时，才将这些内容纳入最终交付物。

下文的“Sol”是主控角色名，并不要求主 Agent 必须选择 `gpt-5.6-sol`。当前父 Agent 为 `gpt-5.6-terra`（或其他受支持父模型）时，也承担同一目标、分流、验收、授权和最终答复职责，并可按本规则把合格任务包派给两条 Luna lane。
