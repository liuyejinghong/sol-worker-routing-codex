...
When a task can be separated into a clearly bounded, independently completable subtask, use the sol-worker-routing skill. Keep Sol in the main thread to understand the objective, split work, review results, and integrate the final answer. First keep tiny one-action tasks with Sol. Route source-pinned, read-heavy, mechanically checkable evidence work to deepseek_worker, including high-context materials that Sol has already discovered and fixed; open-ended discovery and source judgment remain with Sol. Route bounded code review, module analysis, independent implementation, and focused test diagnosis to luna_worker using Luna Max. This is the chosen routing policy: do not re-run a general model-tier comparison or request fresh approval for each eligible delegation. Start with two workers; the Skill may expand accepted, independent, read-only DeepSeek shards to four, while write-bearing Luna work defaults to one. Give each worker an independent, minimal context. Workers must not change the overall objective or broaden their scope. If a DeepSeek provider or effective worker route is unavailable, do not silently claim that it ran. When installation or route recovery is authorized, the skill owns provider inspection, minimal configuration, secure credential guidance, and a real read-only route probe; do not send the user away to discover the provider schema or installation procedure.
...
当任务可以拆成边界明确、能够独立完成的子任务时，使用 sol-worker-routing skill。Sol 留在主线程，负责理解目标、拆分任务、检查结果和整合输出。极小、一步即可完成的任务先由 Sol 直接处理；来源固定、偏阅读、可机械验收的证据任务交给 deepseek_worker，包括由 Sol 先完成发现并固定下来的高上下文材料；开放式发现和来源判断仍由 Sol 负责。代码审查、模块分析、独立实现、聚焦测试排查等需要非平凡代码理解的边界任务交给使用 Luna Max 的 luna_worker。这是已经选定的路由策略，不要为每次合格委派重新比较模型档位或请求确认。开局先用两个 worker；首批结果验收通过后，Skill 可以把相互独立的只读 DeepSeek 分片扩到四个，涉及写入的 Luna 任务默认只开一个。每个 worker 只接收独立、必要的上下文，不得改变整体目标或自行扩大范围。DeepSeek provider 或实际路由不可用时，不得声称它已经运行。安装或路由修复已获授权时，由 skill 负责检查 provider、完成最小配置、引导安全凭据输入并执行一次真实的只读路由探针；不得让使用者自行研究 provider 格式或安装步骤。
