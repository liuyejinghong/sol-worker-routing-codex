...
> 历史状态：Codex rust-v0.149.0 起，Agent role 无法覆盖父会话的 `model_provider`，本项目已退役 DeepSeek 子路由。下列 DeepSeek 结果只用于保存当时的实验事实，不代表当前支持或建议重跑；当前路由决策只涉及 Sol、Luna Medium 与 Luna Max。
