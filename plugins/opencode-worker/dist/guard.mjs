...
var SPECIALISTS = /* @__PURE__ */ new Set(["oracle", "metis", "momus", "explore", "librarian", "multimodal-looker"]);
var AUXILIARY = /* @__PURE__ */ new Set(["title", "summary", "compaction"]);
...
var readTools = /* @__PURE__ */ new Set(["read", "glob", "grep"]);
var writeTools = /* @__PURE__ */ new Set(["write", "edit"]);
async function checkTool(task, tool, args) {
  ...
  if (tool === "bash") {
    ...
    if (args.workdir && await canonicalTarget(path2.resolve(task.input.directory, String(args.workdir))) !== task.input.directory) throw new Error("OPENCODE_WORKER_SCOPE: shell workdir must be the assigned directory");
    return;
  }
  ...
  const lexical = path2.resolve(task.input.directory, raw);
  const target2 = await canonicalTarget(lexical);
  if (!inside(task.input.directory, lexical) || !inside(task.input.directory, target2)) throw new Error("OPENCODE_WORKER_SCOPE: path escapes assigned directory");
  const components = path2.relative(task.input.directory, target2).split(path2.sep);
  ...
  if (writeTools.has(tool)) {
    if (task.input.mode !== "write" || !task.input.writable_paths.some((scope) => scope.endsWith("/") ? inside(scope, target2) : scope === target2)) throw new Error("OPENCODE_WORKER_SCOPE: file is outside writable_paths");
    if (components.some((p) => [".opencode", ".codex", ".agents", ".omo"].includes(p)) || ["AGENTS.md", "opencode.json", "opencode.jsonc"].includes(path2.basename(target2))) throw new Error("OPENCODE_WORKER_SCOPE: execution configuration is protected");
    if (await canonicalTarget(lexical) !== target2) throw new Error("OPENCODE_WORKER_SCOPE: path changed during validation");
  }
}

// src/tree-guard.ts
var denied = (message) => {
  throw new Error(`OPENCODE_WORKER_SCOPE: ${message}`);
};
var TreeGuard = class {
  constructor(file, client, control) {
    this.file = file;
    this.client = client;
  ...
  async locked(fn) {
    const next = this.access.then(fn);
    this.access = next.catch(() => {
    });
    return next;
  }
  queue = Promise.resolve();
  get stateFile() {
    return path3.join(path3.dirname(this.file), "tree.json");
  }
  async persist() {
    this.queue = this.queue.then(() => atomicJson(this.stateFile, this.control));
    await this.queue;
  }
  async task() {
    const task = await readJson(this.file);
    if (!task.routing) throw new Error("Profile routing is required by this guard");
    if (this.control.turn !== task.turn) {
      this.control.turn = task.turn;
      this.control.dispatches = {};
      this.control.violations = [];
      this.control.requests = [];
      this.control.rootWrites = [];
    }
    return task;
  }
  ...
  async bind(sessionID, name) {
    ...
    if (sessionID === task.session_id) {
      ...
      const binding2 = { sessionID, role, route: task.routing.agents.sisyphus, writer: task.input.mode === "write", turn: task.turn, messageID: task.message_id };
      this.control.bindings[sessionID] = binding2;
      return binding2;
    }
    const session = await this.inspect(sessionID);
    if (session.parentID !== task.session_id || path3.resolve(session.directory || "") !== task.input.directory) denied("only direct child sessions in the assigned directory are allowed");
    ...
    const binding = { sessionID, parentID: session.parentID, role, category: dispatch.category, route: dispatch.route, writer: dispatch.writer, turn: task.turn, nativePermission: session.permission };
    this.control.bindings[sessionID] = binding;
    await this.persist();
    return binding;
  }
  async parameters(input, output) {
    const task = await this.task();
  ...
  async activeWriter() {
    ...
    for (const dispatch of Object.values(this.control.dispatches)) {
      ...
      if (dispatch.childID) {
        ...
        const assistants = (response.data || []).filter((m) => m.info?.role === "assistant" && m.info?.parentID === binding?.messageID);
        const last = assistants.at(-1);
        if (binding?.messageID && last?.info.time?.completed && last.info.finish && !["tool-calls", "unknown"].includes(last.info.finish) && !assistants.some((m) => m.parts?.some((p) => p.type === "tool" && ["pending", "running"].includes(p.state?.status)))) {
          dispatch.done = true;
          continue;
        }
      }
  ...
  async before(input, args) {
    ...
    if (input.tool === "task") {
      ...
      const prior = priorID ? this.control.bindings[priorID] : void 0;
      ...
      const dispatch = { callID: input.callID, role, category, route, writer, childID: priorID, done: false };
      this.control.dispatches[input.callID] = dispatch;
      if (prior) {
        prior.turn = task.turn;
        prior.messageID = void 0;
        prior.startMessageID = void 0;
      }
      await this.persist();
      return;
    }
    ...
    if (root && ["write", "edit", "bash"].includes(input.tool)) {
      (this.control.rootWrites ||= []).push(input.callID);
      await this.persist();
    }
  }
  async after(input, output) {
    ...
    if (!dispatch.childID) {
      dispatch.done = true;
      dispatch.error = "Delegation did not return an owned child session";
  ...
  async event(event) {
    ...
    if (event.type === "message.part.updated" && part?.type === "tool" && part.tool === "task" && part.state?.status === "error") {
      ...
      if (dispatch) {
        dispatch.done = true;
        dispatch.error = String(part.state.error || "delegation failed");
        await this.persist();
      }
    }
  }
...
async function loadTreeGuard(file, client) {
  const state = await maybeJson(path3.join(path3.dirname(file), "tree.json"));
  return new TreeGuard(file, client, state);
}

// src/guard.ts
async function OpenCodeWorkerGuard(ctx) {
  ...
  return {
    ...
    "chat.params": async (input, output) => guard.checked(() => guard.parameters(input, output)),
    "tool.execute.before": async (input, output) => guard.checked(() => guard.locked(() => guard.before(input, output.args))),
    "tool.execute.after": async (input, output) => guard.checked(() => guard.locked(() => guard.after(input, output))),
    event: async ({ event }) => guard.event(event)
  };
}
export {
  OpenCodeWorkerGuard as default
};
