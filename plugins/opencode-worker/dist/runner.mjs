...
var startSchema = external_exports.object({
  ...
  profile: external_exports.string().regex(/^[A-Za-z0-9_-]{1,64}$/).default("codex-worker"),
  model_mode: external_exports.enum(["omo", "single"]).default("omo"),
  directory: external_exports.string().min(1),
  task: external_exports.string().min(1).max(6e4),
  mode: external_exports.enum(["read-only", "write"]).default("read-only"),
  writable_paths: external_exports.array(external_exports.string().min(1)).max(200).default([]),
  allowed_commands: external_exports.array(external_exports.string().min(1).max(2e3)).max(30).default([]),
  timeout_seconds: external_exports.number().int().min(30).max(7200).default(900)
}).strict().refine(
  (a) => a.mode === "write" || a.writable_paths.length === 0 && a.allowed_commands.length === 0,
...
var taskDir = (root, id) => path.join(root, "tasks", taskId.parse(id));
var taskFile = (root, id) => path.join(taskDir(root, id), "task.json");
function permissionRules(input, worktree) {
  const rules = [
...
// src/routing.ts
var AGENTS = ["sisyphus", "atlas", "prometheus", "oracle", "metis", "momus", "explore", "librarian", "multimodal-looker", "sisyphus-junior"];
var target = (r) => `${r.providerID}/${r.modelID}`;
...
function validateMaximum(route, models) {
...
function opencodePath() {
  return process.env.OPENCODE_WORKER_BIN || path2.join(os.homedir(), ".opencode/bin/opencode");
}
function api(connection, route, body, timeout = 15e3) {
  ...
  return new Promise((resolve, reject) => {
    const request = http.request(url, {
      ...
      response.on("data", (chunk) => {
        content += chunk;
        if (content.length > 32e6) request.destroy(new Error("OpenCode response exceeds 32 MB; task status needs inspection"));
      });
      response.on("end", () => {
        ...
        try {
          resolve(content ? JSON.parse(content) : void 0);
        } catch {
          reject(new Error("OpenCode returned invalid JSON"));
        }
      });
...
var Runtime = class _Runtime {
  ...
  static async start(directory, artifactDir, taskFile2, routing) {
    await fs2.mkdir(artifactDir, { recursive: true, mode: 448 });
    const port = await unusedPort();
    const logPath = path2.join(artifactDir, "server.log");
    const log = await fs2.open(logPath, "a", 384);
    const password = randomBytes(32).toString("hex");
    const guard = pathToFileURL(fileURLToPath(new URL("./guard.mjs", import.meta.url))).href;
    const config = {
      model: target(routing.agents.sisyphus),
      small_model: target(routing.auxiliary),
      autoupdate: false,
      share: "disabled",
    ...
    const child = spawn(opencodePath(), ["serve", "--hostname", "127.0.0.1", "--port", String(port)], {
      ...
      env: {
        ...
        OMO_PROFILE: routing.profile,
        OPENCODE_SERVER_PASSWORD: password,
        OPENCODE_SERVER_USERNAME: "opencode",
        OPENCODE_WORKER_TASK_FILE: taskFile2
      }
    });
    await log.close();
    let spawnError;
    ...
    const runtime = new _Runtime({ url: "", password, pid: child.pid || 0, directory }, child);
    try {
      if (child.pid) await atomicJson(path2.join(artifactDir, "connection.json"), runtime.connection);
      const deadline = Date.now() + 45e3;
      while (Date.now() < deadline) {
        if (spawnError) throw new Error(`Cannot start OpenCode at ${opencodePath()}: ${spawnError.code || spawnError.message}`);
        if (child.exitCode !== null) throw new Error(`OpenCode exited during startup; inspect ${logPath}`);
        const logText = await fs2.readFile(logPath, "utf8");
        ...
        if (match) {
          ...
          await atomicJson(path2.join(artifactDir, "connection.json"), runtime.connection);
          await atomicJson(path2.join(artifactDir, "network.json"), { inherited_names: ["HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "NO_PROXY", "http_proxy", "https_proxy", "all_proxy", "no_proxy"].filter((key) => Boolean(process.env[key])) });
          return runtime;
        }
        await sleep(100);
      }
  ...
  async inspect(routing) {
    const [health, config, agents, providers, paths] = await Promise.all([
      api(this.connection, "/global/health"),
      api(this.connection, "/config", void 0, 45e3),
      api(this.connection, "/agent", void 0, 45e3),
      api(this.connection, "/provider", void 0, 45e3),
      api(this.connection, "/path")
    ]);
    const chosen = process.env.OPENCODE_WORKER_AGENT;
    const candidates = agents.filter((a) => !a.hidden && a.mode === "primary" && (chosen ? a.name === chosen : /^sisyphus(?:\s|$)/i.test(a.name)));
    ...
    const provider = providers.all.find((p) => p.id === PROVIDER);
    ...
    for (const name of AGENTS) {
      const resolved = agents.find((a) => canonicalAgent(a.name) === name);
    ...
    const plugins = (config.plugin || []).map((p) => typeof p === "string" ? p : JSON.stringify(p));
    if (!plugins.some((p) => p.includes("guard.mjs"))) throw new Error("OpenCode task guard was not loaded");
  ...
  async stop() {
    ...
    if (this.child.exitCode === null && this.child.signalCode === null) {
      ...
      const killDeadline = Date.now() + 2e3;
...
// src/tree.ts
function currentMessages(messages, firstMessageID) {
...
async function descendants(connection, root) {
  const out = [], queue = [root], seen = /* @__PURE__ */ new Set([root]);
  while (queue.length) {
    const parent = queue.shift();
    const children = await api(connection, `/session/${parent}/children`);
...
async function abortTree(connection, root) {
...
function verifyIdentities(row, identities) {
  return identities.every((i) => canonicalAgent(i.agent || "") === row.role && i.providerID === row.providerID && i.modelID === row.modelID);
}
async function treeControl(file) {
  return maybeJson(path3.join(path3.dirname(file), "tree.json"));
}

// src/runner.ts
function summarize(messages, messageId) {
  const assistants = messages.filter((m) => m.info?.role === "assistant" && m.info?.parentID === messageId);
  return {
    text: assistants.flatMap((m) => (m.parts || []).filter((p) => p.type === "text").map((p) => p.text)).join("\n\n"),
    ...
    tools: assistants.flatMap((m) => (m.parts || []).filter((p) => p.type === "tool").map((p) => ({
      tool: p.tool,
...
function terminalState(messages, messageId, runtimeState) {
  ...
  const unresolved = result.tools.some((tool, index) => {
    if (tool.status !== "error" && (tool.exit_code === void 0 || tool.exit_code === 0)) return false;
    ...
    return !result.tools.slice(index + 1).some((later) => {
      if (later.status !== "completed" || later.exit_code !== void 0 && later.exit_code !== 0) return false;
      const next = later.input;
...
async function snapshot(task, target2) {
  const files = [];
  await fs3.rm(target2, { recursive: true, force: true });
  await fs3.mkdir(target2, { recursive: true, mode: 448 });
  const visit2 = async (file) => {
    let st;
    try {
      st = await fs3.lstat(file);
    } catch (e) {
      if (e.code === "ENOENT") return;
      throw e;
    }
    ...
    if (st.isDirectory()) {
      for (const item of await fs3.readdir(file)) await visit2(path4.join(file, item));
    } else if (st.isFile()) {
      if (st.size > 2e7) throw new Error("Owned file exceeds the 20 MB snapshot limit");
      const relative = path4.relative(task.input.directory, file);
      const dest = path4.join(target2, relative);
      await fs3.mkdir(path4.dirname(dest), { recursive: true, mode: 448 });
      await fs3.copyFile(file, dest);
      files.push(relative);
    }
  };
  for (const scope of task.input.writable_paths) await visit2(scope);
  return [...new Set(files)];
}
async function runCommand(command, args, cwd) {
  return new Promise((resolve, reject) => {
    const child = spawn2(command, args, { cwd, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
...
async function saveChanges(task, folder, before) {
  const after = await snapshot(task, path4.join(folder, "after"));
  const changed = [];
  for (const file of /* @__PURE__ */ new Set([...before, ...after])) {
    const read = (side) => fs3.readFile(path4.join(folder, side, file)).catch((e) => {
      if (e.code === "ENOENT") return;
      throw e;
    });
  ...
  await fs3.writeFile(path4.join(folder, "diff.patch"), diff.stdout, { mode: 384 });
  task.result = { ...task.result || { text: "", tools: [], errors: [], identity: [], observed_states: [], acceptance: "pending" }, changed_files: changed, diff_path: path4.join(folder, "diff.patch") };
}
async function stopOrphan(connection, sessionId) {
  try {
    await api(connection, "/global/health");
  } catch (e) {
  ...
  for (let i = 0; i < 40; i++) {
    await sleep(100);
    try {
      await api(connection, "/global/health", void 0, 1e3);
    } catch {
      return;
    }
  }
...
async function execute(root, id, recovery) {
  const file = taskFile(root, id);
  let task = await readJson(file);
  for (let i = 0; task.runner_pid !== process.pid && i < 100; i++) {
    await sleep(50);
    task = await readJson(file);
  }
  if (task.runner_pid !== process.pid) throw new Error("Runner reservation was not acknowledged");
  const folder = path4.join(taskDir(root, id), `turn-${task.turn}`);
  const cancelFile = path4.join(folder, "cancel.json");
  const save = async () => {
    task.updated_at = now();
    await atomicJson(file, task);
  };
  if (recovery) {
    const connection = await maybeJson(path4.join(folder, "connection.json"));
    if (connection && !connection.url) {
      const log = await fs3.readFile(path4.join(folder, "server.log"), "utf8").catch(() => "");
    ...
    try {
      ...
      task.reason = "Orphaned private service stopped; review existing changes before any retry";
      task.finished_at = now();
      ...
      const beforeFiles = [];
      const list = async (base, rel = "") => {
        for (const item of await fs3.readdir(path4.join(base, rel), { withFileTypes: true }).catch(() => [])) {
          const name = path4.join(rel, item.name);
          if (item.isDirectory()) await list(base, name);
          else if (item.isFile()) beforeFiles.push(name);
        }
      };
      await list(path4.join(folder, "before"));
  ...
  try {
    before = await snapshot(task, path4.join(folder, "before"));
    snapshotReady = true;
    if (await maybeJson(cancelFile)) {
      final = "cancelled";
      return;
    }
    ...
    await atomicJson(path4.join(folder, "runtime.json"), { ...metadata, target: target(task.routing.agents.sisyphus), routing: task.routing });
    const permissions = permissionRules(task.input, metadata.worktree);
    if (!task.session_id) {
      const session2 = await api(runtime.connection, "/session", { title: `Codex worker ${id.slice(0, 8)}`, permission: permissions });
      task.session_id = session2.id;
    }
    ...
    await save();
    if (await maybeJson(cancelFile) || signalReceived) {
      final = "cancelled";
      return;
    }
    task.submission = "attempted";
    await save();
    const prompt = [
...
Exact allowed shell commands: ${JSON.stringify(task.input.allowed_commands)}`,
    ...
    await api(runtime.connection, `/session/${task.session_id}/prompt_async`, {
      agent: task.identity.agent,
      model: { providerID: task.identity.providerID, modelID: task.identity.modelID },
      variant: task.routing.agents.sisyphus.reasoning,
      messageID: task.message_id,
      parts: [{ type: "text", text: prompt }]
    });
    task.submission = "acknowledged";
    await save();
    const deadline = Date.now() + task.input.timeout_seconds * 1e3;
    const states = [];
    while (true) {
      const cancelled = Boolean(await maybeJson(cancelFile)) || signalReceived;
      if (cancelled || Date.now() >= deadline) {
        task.cancelling = true;
        task.reason = cancelled ? "Cancellation requested" : "Task time budget reached";
        shouldCancel = true;
        await save();
      ...
      const [statuses, rawMessages, children, control] = await Promise.all([
        api(runtime.connection, "/session/status"),
        api(runtime.connection, `/session/${task.session_id}/message`),
        descendants(runtime.connection, task.session_id),
        treeControl(file)
      ]);
      ...
      for (const child of children) {
        ...
        if (!terminal2) pendingChildren = true;
        if (terminal2 && terminal2 !== "completed") childErrors = true;
      }
      task.result.sessions = rows;
      task.state = runtimeState === "idle" && task.result.identity.length === 0 ? "submitted" : "running";
      task.reason = runtimeState === "retry" ? String(statuses[task.session_id]?.message || "OpenCode is retrying") : void 0;
      ...
      if (control?.turn === task.turn && control.violations.length) {
        ...
        task.reason = control.violations[0];
        shouldCancel = true;
        break;
      }
      const pendingQuestion = messages.some((m) => (m.parts || []).some((p) => p.type === "tool" && p.tool === "question" && ["pending", "running"].includes(p.state?.status)));
      if (pendingQuestion) {
        final = "needs_attention";
        task.reason = "Worker requested input; relay it through Codex";
        shouldCancel = true;
        break;
      }
    ...
    if (runtime) {
      ...
      await runtime.stop().catch((e) => {
        stopped = false;
        final = "unknown";
        task.reason = `Service shutdown unconfirmed: ${errorText(e)}`;
      });
    }
    if (snapshotReady) {
      try {
        await saveChanges(task, folder, before);
      } catch (e) {
        if (stopped) final = "needs_attention";
        task.reason = `Result collection failed: ${errorText(e)}`;
      }
    }
    task.state = final;
    ...
    if (task.result?.identity.length && task.result.identity.every((i) => i.providerID === task.identity?.providerID && i.modelID === task.identity?.modelID) && final === "completed") {
      await atomicJson(path4.join(root, "last-verified.json"), { checked_at: now(), task_id: task.id, identity: task.identity, routing: task.routing, sessions: task.result.sessions });
    }
...
if (process.argv[1]?.endsWith("runner.mjs")) {
  execute(process.argv[2], process.argv[3], process.argv.includes("--recover")).catch((error) => {
