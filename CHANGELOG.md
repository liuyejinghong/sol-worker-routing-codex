...
- 主 Agent 默认直接实现核心资金、权限、事务、状态机和恢复；原生与外部 Worker 承接预期已冻结、可独立验收的外围工作。开发包增加独立预期、实际入口和同根因返工接管要求。
- OpenCode Worker 保留根/子角色终止原因与用量，长度截断和空产物进入待检查；每轮保留 runner/guard，避免插件缓存清理破坏执行和恢复。仅在有明确未启动证据时收口未发送失败。
- [本地验证与边界](docs/2026-09-11-worker-retrospective-improvements.md)。补丁版本；安装与模型路由的证据边界见记录。
...
1. 修复 GNU Bash 5.2 下 lane 启停转换在删除旧状态前校验其不存在、导致失败回滚的问题。
2. 安装器在修改受管文件前保存临时恢复记录；同源码、目录和参数重跑可接续已知中断操作，未知内容仍拒绝，普通失败继续回滚。
3. 增加隔离 HOME 的开关、中断恢复和冲突回归测试；不改变真实安装状态或退役路由边界。

## 0.15.0 - 2026-09-08

1. OpenCode Worker 默认改为单次 run，派发和状态等待在程序内部完成；返工可在同一 followup 调用内等待。
2. 同步前台等待窗口、宿主 MCP 超时与 Code Mode 外层 yield，避免默认短等待导致空续等。
3. 明确后台 start 不注册唤醒、窗口到期及 RPC 中断不等于任务结束，保留原请求去重和文件所有权。
4. 新增实际单次 run 的计量事件记录、最终方案和验证证据；不将完整会话 token 总量当作监控消耗。

## 0.14.0 - 2026-09-07

1. 工作流新增可选 `opencode-worker` 插件通道，使用用户授权的 Go Muse Contributor；插件管理执行，Skill 保留分派和验收。
2. 明确工具发现、最近验证、完成与验收、同会话返工及取消后文件所有权；不改变 Luna 开关状态。
3. 独立 OpenCode 插件不属于旧原生跨 Provider workaround；安装器管理范围不变，只登记上一版 Skill 的精确摘要。
4. 新增方案、本机接入记录及插件开发验收记录；插件源码独立维护，不包含 Web 管理页面。
...
2. 在 `gpt-5.6-terra` / `max` 父任务中完成原生 `luna_medium_worker` 路由探针，子任务实际使用 `gpt-5.6-luna` / `medium`；这证明该已启用 lane 可被 Terra 主控调用，但不把一次探针包装成自动委派承诺
3. 安装器将紧邻的已知旧 `sol-worker-routing` Skill 摘要登记为 v0.12 拓扑，避免已验证的旧安装在更新时被误判为未知冲突；未知内容仍保持 fail closed
